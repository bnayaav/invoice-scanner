...
═══════════════════════════════════════════════════
**מחיר עלות (cost_price) — חוקים מדויקים, חובה לבדוק פעמיים!**
═══════════════════════════════════════════════════

1. **cost_price הוא תמיד מחיר ליחידה אחת**, לא סה"כ של השורה.

2. **בחשבונית עברית טיפוסית יש את העמודות:**
   קוד פריט | שם פריט | כמות | מחיר יח' | סה"כ | הנחה
   - קח **"מחיר יח'"** / **"מחיר ליחידה"** / **"עלות ליחידה"** ישירות → cost_price
   - **אסור** לקחת את העמודה "סה"כ" / "סך הכל" / "סכום" כ-cost_price!

3. **אם בחשבונית יש רק עמודת סה"כ של השורה (בלי עמודת מחיר ליחידה):**
   - חלק את הסה"כ בכמות.
   - דוגמה: כמות=5, סה"כ=50 ₪ → cost_price = 10 ₪
   - דוגמה: כמות=3, סה"כ=120 ₪ → cost_price = 40 ₪

4. **בדיקה עצמית חובה לכל מוצר:** ודא ש-(cost_price × quantity) שווה לסה"כ השורה בחשבונית. אם לא מסתדר — בדוק שוב מאיפה לקחת את המספר. זאת הטעות הכי שכיחה.

5. **התעלם לחלוטין משורות:**
   - סך ביניים / סיכום
   - מע"מ
   - משלוח / דמי טיפול
   - הנחות כלליות
...
6. **מע"מ:** אם בחשבונית רשום מחיר ללא מע"מ ומחיר עם מע"מ — קח **תמיד את המחיר ללא מע"מ**.

7. **הנחה שורתית:** אם יש עמודת הנחה ועמודה "מחיר אחרי הנחה" / "מחיר נטו" — קח את המחיר אחרי ההנחה.

8. **מטבע:** חלץ רק את המספר. "$15.99" → 15.99. "₪25" → 25. "120 ש"ח" → 120.
...
export async function onRequestPost({ request, env }) {
  ...
  if (productsArr.length >= 2) {
    try {
      ...
      const groupingPrompt = `הנה רשימת מוצרים מחשבונית. זהה אילו מהם הם בעצם **אותו מוצר בבסיס** רק בווריאציות שונות (צבע שונה, מידה שונה, גרסה משנית), או שורת בונוס "10+1" כשהיא מופיעה כשורה נפרדת עם מחיר 0 או מחיר נמוך משמעותית.
...
החזר JSON תקין בלבד בפורמט:
{
  "groups": [
    {
      "indices": [0, 1],
      "unified_name": "שם המוצר המשותף ללא האטריבוט המבדיל",
      "reason": "אותו כבל בצבע שונה"
    }
  ]
}

═══════════════════════════════════════════════════
**unified_name — חוקים מדויקים (קריטי!)**
═══════════════════════════════════════════════════

unified_name חייב להיות **החיתוך המילולי** של השמות — כלומר רק המילים המשותפות לכל הפריטים בקבוצה, ללא האטריבוט שמבדיל ביניהם (צבע / מידה / גרסה משנית).

**דוגמאות נכונות:**
- "אוזניות NEO 123 שחור" + "אוזניות NEO 123 לבן" → unified_name: "אוזניות NEO 123"
- "כבל USB-C 1 מטר אדום" + "כבל USB-C 1 מטר כחול" → unified_name: "כבל USB-C 1 מטר"
- "כיסוי iPhone 15 שחור" + "כיסוי iPhone 15 שקוף" + "כיסוי iPhone 15 כחול" → unified_name: "כיסוי iPhone 15"
- "מטען 20W לבן מקורי" + "מטען 20W שחור מקורי" → unified_name: "מטען 20W מקורי"

**חוקים מחייבים:**
1. **רק מילים שהופיעו בכל הפריטים בקבוצה** — אל תוסיף מילים חדשות שלא היו בשמות המקוריים.
2. **שמור על הסדר המקורי** של המילים שנשארו.
3. **הסר רק את האטריבוט המבדיל** — כל שאר המילים נשארות כמו שהן.
4. **אל תקצר** דגמים, מספרים, מותגים — רק את האטריבוט המבדיל.
5. אם הפריטים נבדלים ב-2 אטריבוטים (למשל גם צבע וגם מידה) — הסר את שניהם.

═══════════════════════════════════════════════════

**חוקים על הקבוצות עצמן:**
- כלול בקבוצה רק מוצרים שבאמת זהים בבסיס (אותה פונקציונליות, אותו דגם בסיסי)
