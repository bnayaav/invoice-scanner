...
.history-error svg { width: 14px; height: 14px; flex-shrink: 0; margin-top: 1px; color: var(--red); }

.product-card.has-error { border-color: #fca5a5; }
...
.product-error-banner svg { width: 14px; height: 14px; flex-shrink: 0; margin-top: 1px; color: var(--red); }

/* ─── Product status toggle (new / existing) ──────────────── */
.product-status-row {
...
.status-dot.existing { background: var(--green); }
.status-dot.new { background: var(--amber); }

.status-toggle.print-toggle {
  flex: 1.2;
  gap: 4px;
}
.status-toggle.print-toggle.active {
  background: linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%);
  color: #1e40af;
  border-color: #60a5fa;
}
.status-toggle.print-toggle:not(.active) {
  background: #f3f4f6;
  color: #9ca3af;
  border-color: #e5e7eb;
}

.status-toggle.skip-toggle {
  flex: 0.8;
  background: #fef2f2;
  color: #991b1b;
  border-color: #fecaca;
}
.status-toggle.skip-toggle:hover {
  background: #fee2e2;
}

.product-card.is-skipped {
  position: relative;
  opacity: 0.5;
  background: #f9fafb;
}
.product-card.is-skipped > *:not(.skip-overlay) {
  pointer-events: none;
...
.product-card.is-new { border-left: 3px solid var(--amber); }
.product-card.is-existing { border-left: 3px solid var(--green); }
...
.bulk-mark-btn:active { transform: scale(0.97); }
.bulk-mark-btn:hover { background: white; border-color: var(--ink); }
...
.group-suggestion-header {
  display: flex; align-items: flex-start; gap: 10px;
  margin-bottom: 10px;
}
.group-suggestion-header svg {
  width: 22px; height: 22px;
  color: var(--amber-text);
  flex-shrink: 0; margin-top: 2px;
}
.group-suggestion-text { flex: 1; min-width: 0; }
.group-suggestion-text strong {
  display: block;
  font-family: 'Rubik', sans-serif;
  font-size: 14px; font-weight: 700;
  color: var(--amber-text);
...
.btn-merge:active, .btn-keep-separate:active { transform: scale(0.97); }
.btn-merge {
...
/* ─── Modals (barcode scanner, categories) ────────────────── */
.modal-overlay {
...
.barcode-modal, .categories-modal {
...
.barcode-hint {
  text-align: center; margin: 14px;
  font-size: 13px; color: var(--muted);
}
.barcode-queue {
  text-align: center; margin: 0 14px 8px;
  font-size: 12px; font-weight: 600; color: var(--amber-text);
}
.barcode-modal .btn-primary { margin: 0 14px 14px; }

/* Product barcode/category fields */
.product-barcode-row {
  display: grid; grid-template-columns: 1fr auto;
  gap: 8px; margin-top: 10px;
}
.product-barcode-row label {
  font-size: 10px; font-weight: 700;
  letter-spacing: 0.05em; text-transform: uppercase;
  color: var(--muted); display: block; margin-bottom: 4px;
}
.product-barcode-row input {
  width: 100%;
  padding: 9px 12px;
  background: var(--bg-paper);
  border: 1px solid var(--line);
...
.product-barcode-row input:focus { border-color: var(--amber); }
.scan-barcode-btn {
  align-self: end;
  ...
  display: flex; align-items: center; justify-content: center;
  transition: transform 0.1s;
  box-shadow: 0 2px 8px rgba(26, 31, 46, 0.2);
}
.scan-barcode-btn:active { transform: scale(0.92); }
.scan-barcode-btn svg { width: 18px; height: 18px; }

.product-cat-row {
  display: grid; grid-template-columns: 1fr 1fr;
  gap: 8px; margin-top: 8px;
}
.product-cat-row label {
  font-size: 10px; font-weight: 700;
  letter-spacing: 0.05em; text-transform: uppercase;
  color: var(--muted); display: block; margin-bottom: 4px;
}
.product-cat-row select, .product-cat-row input {
  width: 100%;
  padding: 9px 12px;
  background: var(--bg-paper);
  border: 1px solid var(--line);
...
.categories-add-row .btn-primary {
  padding: 11px 22px;
  margin: 0;
}
.categories-list {
...
.category-item span { font-weight: 500; color: var(--ink); }
...
.category-item button svg { width: 14px; height: 14px; }

/* Send-to-POS button in summary */
.btn-summary.pos {
  background: linear-gradient(135deg, #16a34a 0%, #15803d 100%);
  color: white;
}
...
.install-banner svg { color: var(--gold); width: 28px; height: 28px; flex-shrink: 0; }
.install-banner-text { flex: 1; min-width: 0; }
.install-banner-text strong { font-size: 13px; font-weight: 700; display: block; }
.install-banner-text p { font-size: 11px; margin: 2px 0 0; color: rgba(252, 211, 77, 0.7); }
...
.install-banner .icon-btn svg { width: 14px; height: 14px; }
