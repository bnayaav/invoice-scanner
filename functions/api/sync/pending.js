...
export async function onRequestGet({ request, env }) {
  ...
  for (const inv of invoices) {
    const { results: products } = await env.DB
      .prepare(`SELECT id, name, model, quantity, cost_price, customer_price, sort_order, barcode, category, supplier_name, is_new, print_labels, skip_import, print_only, previous_cost
